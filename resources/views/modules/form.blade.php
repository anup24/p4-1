    ...
    <div class='form-group'>
        ...
        <select name='sourceLanguage' id='sourceLanguage' class='form-control'>
            @foreach ($srcLang as $lang => $arr)
                <option
                    value='{{ $arr->short_name }}'{{ ($arr->short_name == old('sourceLanguage')) ? ' selected' : '' }}>{{ $arr->name }}</option>
            @endforeach
        </select>
        ...
        <select name='targetLanguage' id='targetLanguage' class='form-control'>
            @foreach ($targetLang as $lang => $arr)
                <option
                    value='{{ $arr->short_name }}'{{ ($arr->short_name == old('targetLanguage')) ? ' selected' : '' }}>{{ $arr->name }}</option>
            @endforeach
        </select>
